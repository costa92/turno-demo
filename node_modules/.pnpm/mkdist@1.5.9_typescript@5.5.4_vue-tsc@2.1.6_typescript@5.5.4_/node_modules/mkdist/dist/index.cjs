  ...
  for (const filename of inputFiles) {
    ...
    if (opts?.addRelativeDeclarationExtensions) {
      ...
      for (const spec of [...exports, ...typeExports, ...imports]) {
        ...
        let isDir = false;
        try {
          const declaration = pathe.resolve(
            filename,
            "..",
            spec.specifier + ext.replace(EXT_RE, ".d.$1ts")
          );
          if (!vfs.get(declaration)) {
            isDir = node_fs.statSync(
              pathe.resolve(filename, "..", spec.specifier)
            ).isDirectory();
          }
        } catch {
        }
...
async function emitVueTscLatest(vfs, compilerOptions, srcFiles) {
  const { resolve: resolveModule } = await import('mlly');
  const ts = await import('typescript').then(
    (r) => r.default || r
  );
...
async function mkdist(options = {}) {
  ...
  const filePaths = await fg__default(options.pattern || "**", {
    absolute: false,
    cwd: options.srcDir
  });
